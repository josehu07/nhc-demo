...
#define OCF_ENGINE_DEBUG_IO_NAME "mfwb"
...
/**
 * Below are MFC with write-back - read implementation.
 */
static void _ocf_read_mfwb_to_cache_cmpl(struct ocf_request *req, int error)
{
    if (error)
        req->error |= error;
...
static inline void _ocf_read_mfwb_submit_to_cache(struct ocf_request *req)
{
    ...
    ocf_submit_cache_reqs(req->cache, req, OCF_READ, 0, req->byte_length,
                          ...
                          _ocf_read_mfwb_to_cache_cmpl);
}

static void _ocf_read_mfwb_to_core_cmpl_do_promote(struct ocf_request *req,
                                                   int error)
{
...
static void _ocf_read_mfwb_to_core_cmpl_no_promote(struct ocf_request *req,
                                                   int error)
{
    struct ocf_cache *cache = req->cache;
...
static inline void _ocf_read_mfwb_submit_to_core(struct ocf_request *req,
                                                 bool promote)
{
    struct ocf_cache *cache = req->cache;
    int ret;
...
static int _ocf_read_mfwb_do(struct ocf_request *req)
{
    ...
    if (ocf_engine_is_hit(req)) {
        ...
        /** Hit && p <= load_admit. */
        } else if (req->load_admit_allowed) {
            OCF_DEBUG_RQ(req, "Submit");
            _ocf_read_mfwb_submit_to_cache(req);
        ...
        if (req->data_admit_allowed) {
            ...
            OCF_DEBUG_RQ(req, "Submit");
            _ocf_read_mfwb_submit_to_core(req, true);
...
static enum ocf_engine_lock_type ocf_read_mfwb_get_lock_type(struct ocf_request *req)
{
    if (ocf_engine_is_hit(req)) {
        if (req->info.dirty_any)
...
static const struct ocf_io_if _io_if_read_mfwb_resume = {
    .read = _ocf_read_mfwb_do,
    .write = _ocf_read_mfwb_do,
};

static const struct ocf_engine_callbacks _read_mfwb_engine_callbacks =
{
    .get_lock_type = ocf_read_mfwb_get_lock_type,
    .resume = ocf_engine_on_resume,
};

/**
 * Multi-factor read with write-back.
 *
 * If fully hit && dirty, we read from cache. If fully hit && p <=
 * `load_admit`, we read from cache. Otherwise, we read from core.
 *
...
int ocf_read_mfwb(struct ocf_request *req)
{
    ...
    req->io_if = &_io_if_read_mfwb_resume;

    lock = ocf_engine_prepare_clines(req, &_read_mfwb_engine_callbacks);
